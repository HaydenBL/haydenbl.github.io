...
Motion is gated in one place: `prefersReducedMotion()` in `src/composables/useReducedMotion.ts`, which reads `matchMedia` at call time rather than caching so an OS toggle takes effect without a reload. Under Reduce Motion the entrance collapses to a plain fade and the stagger goes to zero. `useTilt.ts` (the hover tilt/gloss) asks the same helper and re-checks it on `change`. Don't add a new definition of either query. `(any-hover: hover)` already lives in exactly three places and they must move together: Tailwind's `hover:` variant is overridden to it in `index.css`, `useTilt` asks it in JS, and `Item.vue`'s scoped CSS gates the wedge wipe and the `.go` nudge on it. That third copy is unavoidable — scoped CSS never passes through Tailwind, so it cannot reach the variant. `Item.vue` also has a `(hover: none)` press rule that deliberately does *not* match; the comment there says why.
...
- **A touch device gets no wedge growth at all — the `scale(0.985)` press is its only feedback.** The wipe used to fire on phones for free, because iOS latches `:hover` on tap and holds it until you tap elsewhere; that read as the triangle growing at random, which is what prompted the gate. Growing the wedge on `:active` instead was offered as a way to keep the gesture on phones and **declined** — hover means nothing to a finger, so the wedge is pointer-only on purpose. Don't file the empty phone hover as a regression.
- **The hover gate asks the device, not each input.** Gating on `(any-hover: hover)` means an iPad with a trackpad answers yes to everything, so a *finger* tap there still latches hover and still grows the wedge — the same way it already triggers the tilt. Making that inert needs `useTilt` to switch to pointer events and filter on `pointerType === "mouse"`, which was costed and **declined**: it moves the wedge off pure CSS onto JS and rewrites a composable carrying a lot of hard-won behaviour, for one uncommon case. Reopen only if that case actually starts bothering him.
